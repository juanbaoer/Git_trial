...
# folder_name = f"Exp_VL{n_layers_V}_VN{n_neurons_V}_LR{lr}_BS{batchsize}"
...
#     handlers=[
#         logging.FileHandler(os.path.join(folder_name, "log.txt"), mode='w'),
...
def lennard_jones_energy_full(x, n_particles, n_dims, box_length=2*torch.pi, eps=1.0, rm=1.0, r_switch=0.95, r_cutoff=2.5, remove_diagonal=True):
    ...
    a = -5.0  # Given slope for linear part
...
def compute_loss(Velocity, Time_residual, n_particles, n_dimension, batchsize, device="cuda", bound = 2*torch.pi):
    timings = {}

    # Sample collocation points for PDE residual
    n_pde_points = batchsize  # Number of points inside domain
...
# Training function
# def train(n_particles, n_dimension, batchsize, Velocity, Time_residual, optimizer, scheduler, device="cuda", n_epochs=10000, print_every=1000):
...
#         total_loss, pde_loss, ic_loss, bc_loss, loss_timings = compute_loss(Velocity, Time_residual, n_particles, n_dimension, batchsize, device)
...
# logger.info(f"  Dimensions: {n_dimension}")
# logger.info(f"  Velocity model layers: {n_layers_V}")
# logger.info(f"  Velocity model neurons: {n_neurons_V}")
# logger.info(f"  Time model layers: {n_layers_T}")
# logger.info(f"  Time model neurons: {n_neurons_T}")
...
Velocity = PINN2D(
    ...
    feat_dim=0,        
    edge_dim=0,         
    hidden_dim=n_neurons_V,      
    norm_coors_scale_init=1.0,  
    cutoff_distance=torch.pi,  
    num_layers=n_layers_V      
).to(device)

Velocity.load_state_dict(torch.load("LJ_exp/2025_05_21/Exp_VL3_VN32_LR0.01_BS128/velocity_model.pth"),map_location=device)
print(Velocity)
# 